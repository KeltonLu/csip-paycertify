...
@"SELECT counts=count(1) FROM Pay_SV WHERE keyinDay>=@STARTDATE  AND keyinday<=@ENDDATE  AND Void='Y'";
        ...
        public const string SEL_Temp =
                            ...
                            ";
        //* SQL 異常 B.BLK 重複，調整SQL  原   B.CUSID, B.CUSNAME,B.BLK, -> B.CUSID, B.CUSNAME,
        // 修改Ares Neal
        // 創建時間:2022/02/21
        ...
        public static bool Report01010100(string strCustID, ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            try
            {
                ...
                try
                {
                    RecordSQL($"新增清償證明 [{queryTypeName}]", "", "Pay_Macro", sql.GetFilterCondition());
                    sw.Start();
                    esPayMacro.FillEntitySet(sql.GetFilterCondition());
                    sw.Stop();
                    RecordSQLExecuteTime($"新增清償證明 [{queryTypeName}]", sw, esPayMacro.TotalCount);
                }
                catch (Exception exp)
                {
        ...
        /// <param name="queryType">查詢類型S：查詢/P：列印</param>
        /// <returns></returns>
        public static bool Report01020200(string strMailDay, string strMailType, ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            string sqlcmd = "";
            return Report01020200(strMailDay, strMailType, ref strMsgID, ref dstResult, ref sqlcmd, queryType);
        }
        public static bool Report01020200(string strMailDay, string strMailType, ref string strMsgID, ref DataSet dstResult,ref string sqlcmd, string queryType = "S")
        {
            try
            {
                string queryTypeName = queryType == "S" ? "查詢" : "列印";
                ...
                sqlcmd = string.Format("select * from {0} where 1=1 {1}", "Pay_Certify", sql.GetFilterCondition()) ;
                try
                {
                    RecordSQL($"大宗掛號報表(清償證明) 查詢 [{queryTypeName}]", "", "Pay_Certify", sql.GetFilterCondition());
                    sw.Start();
                    esPay_Certify.FillEntitySet(sql.GetFilterCondition());
                    sw.Stop();
                    RecordSQLExecuteTime($"大宗掛號報表(清償證明) 查詢 [{queryTypeName}]", sw, esPay_Certify.TotalCount);
                }
                catch (Exception exp)
                {
                    BRReport.SaveLog(exp);
        ...
        public static bool Report02020600(string strRebackDateStart, string strRebackDateEnd, string strApplyDateStart, string strApplyDateEnd,
                                           string strUserID, ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            try
            {
                string queryTypeName = queryType == "S" ? "查詢" : "列印";
        ...
        public static bool Report0302020002(string strStartSerialNo, string strEndSerialNo, string strUserID,
                    string strMailDay, string strCardNo, ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            string queryTypeName = queryType == "S" ? "查詢" : "列印";
            Stopwatch sw = new Stopwatch();
            ...
            try
            {
                ...
                RecordSQL($"結清證明 列印自取簽收表(列印簽收總表) [{queryTypeName}]", "", "Pay_Certify", Sql.GetFilterCondition());
                sw.Start();
                ...
                sw.Stop();
                RecordSQLExecuteTime($"結清證明 列印自取簽收表(列印簽收總表) [{queryTypeName}]", sw, esReport0302020002.TotalCount);
                    ...
                    string strMailDay, string strCardNo, ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            ...
            try
            {
                ...
                //20220222_Ares_Jack_新增LOG紀錄
                RecordSQL($"結清證明 列印自取簽收表(列印簽收表) [{queryTypeName}]", "", "Pay_Certify", Sql.GetFilterCondition());
                sw.Start();
                ...
                sw.Stop();
                RecordSQLExecuteTime($"結清證明 列印自取簽收表(列印簽收表) [{queryTypeName}]", sw, esReport0302020003.TotalCount);
                    ...
                    string strMailDay, string strCardNo, ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            ...
            try
            {
                ...
                //20220222_Ares_Jack_新增LOG紀錄
                RecordSQL($"代償證明 列印自取簽收表(列印簽收總表) [{queryTypeName}]", "", "Pay_Certify", Sql.GetFilterCondition());
                sw.Start();
                ...
                sw.Stop();
                RecordSQLExecuteTime($"代償證明 列印自取簽收表(列印簽收總表) [{queryTypeName}]", sw, esReport0402020002.TotalCount);
                    ...
                    string strMailDay, string strCardNo, ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            ...
            try
            {
                ...
                //20220222_Ares_Jack_新增LOG紀錄
                RecordSQL($"代償證明 列印自取簽收表(列印簽收表) [{queryTypeName}]", "", "Pay_Certify", Sql.GetFilterCondition());
                sw.Start();
                ...
                sw.Stop();
                RecordSQLExecuteTime($"代償證明 列印自取簽收表(列印簽收表) [{queryTypeName}]", sw, esReport0402020003.TotalCount);
                    ...
                    string strMailDay, string strCardNo, ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            ...
            dtblReport.Columns.Add(new DataColumn("HeadOffice", Type.GetType("System.String")));//總行資料
            ...
            try
            {
                ...
                //20220222_Ares_Jack_新增LOG紀錄
                RecordSQL($"結清證明 列印結清證明書 [{queryTypeName}]", "", "Pay_Certify", Sql.GetFilterCondition());
                sw.Start();
                ...
                if (esReport.TotalCount > 0)
                {
                    for (int intLoop = 0; intLoop < esReport.TotalCount; intLoop++)
                    {
                        ...
                        drowNewInsert["HeadOffice"] = MessageHelper.GetMessage("04_00000000_059");//總行資料

                        dtblReport.Rows.Add(drowNewInsert);
                    }
                }
                sw.Stop();
                RecordSQLExecuteTime($"結清證明 列印結清證明書 [{queryTypeName}]", sw, esReport.TotalCount);
                    ...
                    string strMailDay, string strCardNo, ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            ...
            dtblReport.Columns.Add(new DataColumn("payDay", Type.GetType("System.String")));
            //總行資料
            dtblReport.Columns.Add(new DataColumn("HeadOffice", Type.GetType("System.String")));
            ...
            try
            {
                ...
                EntitySet<EntitySet_Pay_Certify> esReport = (EntitySet<EntitySet_Pay_Certify>)BRSet_Pay_Certify.Search(Sql.GetFilterCondition());
                //20220222_Ares_Jack_新增LOG紀錄
                RecordSQL($"代償證明 列印代償證明書 [{queryTypeName}]", "", "Pay_Certify", Sql.GetFilterCondition());
                sw.Start();
                ...
                sw.Stop();
                RecordSQLExecuteTime($"代償證明 列印代償證明書 [{queryTypeName}]", sw, esReport.TotalCount);
        ...
        public static bool Report0302060002(string strMailDay, string strMailType, string strLogonUserName,
                    ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            string queryTypeName = queryType == "S" ? "查詢" : "列印";
            Stopwatch sw = new Stopwatch();
            SqlHelper Sql = new SqlHelper();
            ...
            try
            {
                ...
                //20220222_Ares_Jack_新增LOG紀錄
                RecordSQL($"結清證明 列印大宗掛號函件存根聯 [{queryTypeName}]", "", "Pay_Certify", Sql.GetFilterCondition());
                sw.Start();
                ...
                sw.Stop();
                RecordSQLExecuteTime($"結清證明 列印大宗掛號函件存根聯 [{queryTypeName}]", sw, esReport0302060002.TotalCount);
                    ...
                    ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            ...
            try
            {
                ...
                //20220222_Ares_Jack_新增LOG紀錄
                RecordSQL($"代償證明 列印大宗掛號函件存根聯 [{queryTypeName}]", "", "Pay_Certify", Sql.GetFilterCondition());
                sw.Start();
                ...
                sw.Stop();
                RecordSQLExecuteTime($"代償證明 列印大宗掛號函件存根聯 [{queryTypeName}]", sw, esReport0402060002.TotalCount);
                    ...
                    ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            ...
            try
            {
                ...
                //20220222_Ares_Jack_新增LOG紀錄
                RecordSQL($"結清證明 列印ML結清證明開立登記表 [{queryTypeName}]", "", "Pay_Certify", Sql.GetFilterCondition());
                sw.Start();
                ...
                sw.Stop();
                RecordSQLExecuteTime($"結清證明 列印ML結清證明開立登記表 [{queryTypeName}]", sw, esReport0302030002.TotalCount);
                    ...
                    ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            ...
            try
            {
                ...
                //20220222_Ares_Jack_新增LOG紀錄
                RecordSQL($"代償證明 列印ML代償證明開立登記表 [{queryTypeName}]", "", "Pay_Certify", Sql.GetFilterCondition());
                sw.Start();
                ...
                sw.Stop();
                RecordSQLExecuteTime($"代償證明 列印ML代償證明開立登記表 [{queryTypeName}]", sw, esReport0402030002.TotalCount);
        ...
        public static bool ReportPaySV(string strSerialNoList, ref string strMsgID, ref DataSet dstResult)
        {
            ...
            dtblPayCertificate.Columns.Add(new DataColumn("HeadOffice", Type.GetType("System.String")));//總行資料
            ...
            try
            {
                for (int i = 0; i < dtblPaySV.Rows.Count; i++)
                {
                    ...
                    drowPayCertificate["HeadOffice"] = MessageHelper.GetMessage("04_00000000_059");//總行資料
        ...
        /// <param name="queryType">查詢類型S：查詢/P：列印</param>
        /// <returns>返回執行標識：True---執行成功；False----執行失敗</returns>
        public static bool Report02020300_2(string strUserID, string strBeforeDate, string strEndDate, bool blIsKeyIn, ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            string queryTypeName = queryType == "S" ? "查詢" : "列印";
            Stopwatch sw = new Stopwatch();
            ...
            //20220223_Ares_Jack_新增LOG紀錄
            BRReport.RecordSQL($"債清客戶 列印債清證明 [{queryTypeName}]", BRReport.GetFullSqlCmd(sqlcmdSearchResult));
            sw.Start();
            ...
            dtblPaySV = dstPaySV.Tables[0];
            dataCount = dtblPaySV.Rows.Count;
            ...
            BRReport.RecordSQLExecuteTime($"債清客戶 列印債清證明 [{queryTypeName}]", sw, dataCount);
            return ReportPaySV(strSerialNoList, ref strMsgID, ref dstResult);
        }
        ...
        /// <param name="queryType">查詢類型S：查詢/P：列印</param>
        /// <returns>返回執行標識：True---執行成功；False----執行失敗</returns>
        public static bool Report01020300_2(string strUserID, string strType, string strStartDate, string strEndDate, ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            SqlHelper sql = new SqlHelper();
            ...
            Logging.Log("全部清償證明 查詢/列印", LogState.Info, LogLayer.Util);
            return ReportPayCertify(sql, ref strMsgID, ref dstResult, queryType, "清償證明 列印清償證明");
        }
        ...
        public static bool Report01020300(string strStartSerialNo, ref string strMsgID, ref DataSet dstResult)
        {
            ...
            Logging.Log("個別清償證明 查詢/列印", LogState.Info, LogLayer.Util);
        ...
        /// <returns>返回執行標識：True---執行成功；False----執行失敗</returns>
        public static bool ReportPayCertify(SqlHelper sqlCondition, ref string strMsgID, ref DataSet dstResult, string queryType = "S", string reportName = "")
        {
            string queryTypeName = queryType == "S" ? "查詢" : "列印";
            ...
            try
            {
                RecordSQL($"{reportName} [{queryTypeName}]", "", "Pay_Certify", sqlCondition.GetFilterCondition());
                sw.Start();
                esPayCertify.FillEntitySet(sqlCondition.GetFilterCondition());
                sw.Stop();
                RecordSQLExecuteTime($"{reportName} [{queryTypeName}]", sw, esPayCertify.TotalCount);
            }
            catch (Exception exp)
            {
                BRReport.SaveLog(exp);
        ...
        /// <param name="queryType">查詢類型S：查詢/P：列印</param>
        /// <returns>返回執行標識：True---執行成功；False----執行失敗</returns>
        public static bool Report01020100(string strBeforeDate, string strEndDate, ref string strMsgID, ref DataSet dstResult, string queryType = "S")
        {
            string sqlcom = "";
            return Report01020100(strBeforeDate, strEndDate, ref strMsgID, ref dstResult, ref sqlcom,  queryType );
        }

        public static bool Report01020100(string strBeforeDate, string strEndDate, ref string strMsgID, ref DataSet dstResult, ref string sqlcmd, string queryType = "S")
        {
            try
            {
                string queryTypeName = queryType == "S" ? "查詢" : "列印";
                ...
                RecordSQL($"列印/查詢 清償證明統計表-明細資料查詢 [{queryTypeName}]", GetFullSqlCmd(sqlcmdCertify));

                sqlcmd = string.Format(GetFullSqlCmd(sqlcmdCertify));
                sw = new Stopwatch();
                sw.Start();
                DataTable dtblPayCertify = BRReport.SearchOnDataSet(sqlcmdCertify).Tables[0];
                sw.Stop();
                RecordSQLExecuteTime($"列印/查詢 清償證明統計表-明細資料查詢 [{queryTypeName}]", sw, dtblPayCertify.Rows.Count);
                ...
                RecordSQL($"列印/查詢 清償證明統計表-總件數查詢 [{queryTypeName}]", GetFullSqlCmd(cmdTotal));
                sw.Reset();
                sw.Start();
                iTotalCount = BRReport.SearchOnDataSet(cmdTotal).Tables[0].Rows.Count;   //* 總件數
                sw.Stop();
                RecordSQLExecuteTime($"列印/查詢 清償證明統計表-總件數查詢 [{queryTypeName}]", sw, iTotalCount);
                ...
                RecordSQL($"列印/查詢 清償證明統計表-免費手續費件數(不含註銷)查詢 [{queryTypeName}]", GetFullSqlCmd(cmdFree));
                sw.Reset();
                sw.Start();
                iFreeCount = int.Parse(BRReport.SearchOnDataSet(cmdFree).Tables[0].Rows[0]["counts"].ToString());
                sw.Stop();
                RecordSQLExecuteTime($"列印/查詢 清償證明統計表-免費手續費件數(不含註銷)查詢 [{queryTypeName}]", sw, iFreeCount);
                ...
                RecordSQL($"列印/查詢 清償證明統計表-收手續費件數(不含註銷)查詢 [{queryTypeName}]", GetFullSqlCmd(cmdNotFree));
                sw.Reset();
                sw.Start();
                iNoFreeCount = int.Parse(BRReport.SearchOnDataSet(cmdNotFree).Tables[0].Rows[0]["counts"].ToString());
                sw.Stop();
                RecordSQLExecuteTime($"列印/查詢 清償證明統計表-免費手續費件數(不含註銷)查詢 [{queryTypeName}]", sw, iNoFreeCount);
                ...
                RecordSQL($"列印/查詢 清償證明統計表-退回筆數查詢 [{queryTypeName}]", GetFullSqlCmd(cmdReutrn));
                sw.Reset();
                sw.Start();
                iReutrnCount = int.Parse(BRReport.SearchOnDataSet(cmdReutrn).Tables[0].Rows[0]["counts"].ToString());
                sw.Stop();
                RecordSQLExecuteTime($"列印/查詢 清償證明統計表-退回筆數查詢 [{queryTypeName}]", sw, iReutrnCount);
                ...
                RecordSQL($"列印/查詢 清償證明統計表-註銷件數查詢 [{queryTypeName}]", GetFullSqlCmd(cmdCacnel));
                sw.Reset();
                sw.Start();
                iCancelCount = int.Parse(BRReport.SearchOnDataSet(cmdCacnel).Tables[0].Rows[0]["counts"].ToString());
                sw.Stop();
                RecordSQLExecuteTime($"列印/查詢 清償證明統計表-註銷件數查詢 [{queryTypeName}]", sw, iCancelCount);
        ...
        /// <param name="queryType">查詢類型S：查詢/P：列印</param>
        /// <returns>返回執行標識：True---執行成功；False----執行失敗</returns>
        public static bool Report02020100(string strBeforeDate, string strEndDate, ref string strMsgID, ref DataSet dstResult, int PageIndex = 0, int PageSize = 0, string queryType = "S")
        {
            string sqlcmd = "";
            return Report02020100( strBeforeDate,  strEndDate, ref  strMsgID, ref  dstResult,ref sqlcmd, PageIndex, PageSize, queryType);
        }
        public static bool Report02020100(string strBeforeDate, string strEndDate, ref string strMsgID, ref DataSet dstResult, ref string sqlcmd, int PageIndex = 0, int PageSize = 0, string queryType = "S")
        {
            string queryTypeName = queryType == "S" ? "查詢" : "列印";
            ...
            try
            {
                sqlcmd = string.Format(GetFullSqlCmd(sqlcmdsum));
                //* 取得所有資料

                Stopwatch sw = new Stopwatch();
                sw.Start();                RecordSQL($"債清客戶 債清證明統計表 [{queryTypeName}]", GetFullSqlCmd(sqlcmdsum));
                int TotalCount = 0;

                if(PageIndex !=0)
                {
                  dtblSum = BRReport.SearchOnDataSet(sqlcmdsum,PageIndex,PageSize,ref  TotalCount).Tables[0];
                }
                else
                {
                ...
                RecordSQLExecuteTime($"債清客戶 債清證明統計表 [{queryTypeName}]", sw, dtblSum.Rows.Count);
                sw.Reset();
        ...
        /// <param name="prefix">前後綴</param>
        /// <param name="fileName">檔案名稱</param>
        /// <param name="templatePath">範本檔位置</param>
        /// <param name="outputDirectory">匯出檔案位置</param>
        /// <returns></returns>
        public static string ExportWordFile(DataSet dataSource, string prefix, string fileName, string templatePath, string outputDirectory)
        {
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(para.ParagraphText))
                    {
                        for (int i = 0; i < dataSource.Tables[0].Rows.Count; i++)
                        {
                            para.ReplaceText(prefix + dataSource.Tables[0].Columns[i] + prefix, dataSource.Tables[0].Rows[0].ItemArray[i].ToString());
                        }
                    }
                }
                Function.CheckDirectory(ref outputDirectory);
                string strServerPathFile = outputDirectory + fileName;
                FileStream output = new FileStream(strServerPathFile, FileMode.Create);
                doc.Write(output);
                fs.Close();
                fs.Dispose();
                output.Close();
        ...
        public static string GetFullSqlCmd(SqlCommand sqlCmd)
        {
            try
            {
                for (int i = 0; i < sqlCmd.Parameters.Count; i++)
                {
                    sqlCmd.CommandText = sqlCmd.CommandText.Replace(sqlCmd.Parameters[i].ParameterName.ToString(), string.Format("'{0}'", sqlCmd.Parameters[i].Value.ToString()));
                }
                return sqlCmd.CommandText;
            }
            catch (Exception ex)
            {
                return "取得完整SQL時發生錯誤：" + ex.ToString();
            }
        }

        /// <summary>
        /// 紀錄SQL
        /// 修改紀錄：log調整 by Ares Stanley 20220406
        /// </summary>
        /// <param name="message"></param>
        /// <param name="sqlCmd"></param>
        /// <param name="tableName"></param>
        /// <param name="condition"></param>
        public static void RecordSQL(string message, string sqlCmd, string tableName = "", string condition = "")
        {
            try
            {
                if (!string.IsNullOrEmpty(tableName) && !string.IsNullOrEmpty(condition))
                {
                    Logging.Log(string.Format("{0}：\r\nselect * from {1} where 1=1 {2}", message, tableName, condition));
                }
                else
                {
                    Logging.Log(string.Format("{0}：\r\n{1}", message, sqlCmd), LogState.Info, LogLayer.Util);
                }

            }
            catch (Exception ex)
            {
                Logging.Log("記錄SQL時發生錯誤：" + ex.ToString());
            }
        }

        public static void RecordSQLExecuteTime(string message, Stopwatch sw, int dataCount)
        {
            try
            {
                TimeSpan ts = sw.Elapsed;
                Logging.Log(string.Format("{0}：\r\n執行時間共：{1} ms，共 {2} 筆資料", message, ts.TotalMilliseconds, dataCount), LogState.Info, LogLayer.Util);
            }
            catch (Exception ex)
            {
                Logging.Log("記錄SQL執行時間時發生錯誤：" + ex.ToString());
            }
        }
