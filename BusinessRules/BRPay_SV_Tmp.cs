...
Case_No = @Case_No";
        ...
        /// <summary>
        /// 查詢未驗證+已驗證未開立資料
        /// 調整紀錄：增加try catch by Ares Stanley 20220214
        /// </summary>
        ...
        public static bool SearchNoAddInfo(ref string strRtnMsg, ref DataTable dtblResult)
        {
            try
            {
                ...
                BRReport.RecordSQL("查詢未驗證+已驗證未開立資料", BRReport.GetFullSqlCmd(sqlcmd));
                sw.Start();
                ...
                BRReport.RecordSQLExecuteTime("查詢未驗證+已驗證未開立資料", sw, 0);
                return true;
            }
            catch(Exception ex)
            {
                Logging.Log(ex);
                strRtnMsg = "04_02010100_002"; //查詢未開立明細失敗
                return false;
            }
        ...
        public static bool SearchRebackInfo(ref string strRtnMsg, ref DataTable dtblResult)
        {
            try
            {
                ...
                //20220222_Ares_Jack_新增LOG紀錄
                BRReport.RecordSQL("查詢剔退明細資料", BRReport.GetFullSqlCmd(sqlcmd));
                sw.Start();
                dtblResult = BRPay_SV_Tmp.SearchOnDataSet(sqlcmd).Tables[0];
                sw.Stop();
                if (dtblResult == null)
                ...
                BRReport.RecordSQLExecuteTime("查詢剔退明細資料", sw, 0);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public static bool AddAll(ref string strRtnMsg, ref string strSerialNoList)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(strSerialNoList))
                {
                    ...
                    Logging.Log(strSerialNoList, LogState.Info, LogLayer.Util);
                }
                return true;
            }
            catch (Exception Ex)
