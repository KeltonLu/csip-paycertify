...
namespace CSIPPayCertify.BusinessRules
{
    public class BRSet_Pay_Certify : CSIPCommonModel.BusinessRules.BRBase<EntitySet_Pay_Certify>
    {
        ...
        private static string SEL_PAY_CERTIFY_03020100 = @" SELECT DISTINCT " +
                                                           ...
                                                           "     ,A.UserID " +
                                                           " FROM " +
                                                           "     " + UtilHelper.GetAppSettings("ServerPaycheck") + @"..[Set_Pay_Certify] AS A " + 
                                                           "     LEFT OUTER JOIN " +
                                                           "     " + UtilHelper.GetAppSettings("ServerCSIP") + @"..[M_USER] AS B " + 
        ...
        public static bool SearchResult_03020100(String strStartSerialNo, String strEndSerialNo, 
                ...
                ref string strMsgID,ref  DataTable dtblResult, string queryType = "S")
         {
             try
             {
                ...
                BRReport.RecordSQL($"結清證明 查詢已開立的結清證明 [{queryTypeName}]", BRReport.GetFullSqlCmd(sqlcmdSearchResult));
                sw.Start();
                ...
                BRReport.RecordSQLExecuteTime($"結清證明 查詢已開立的結清證明 [{queryTypeName}]", sw, dataCount);
                return true;
             }
             catch(Exception exp)
             {
        ...
        public static bool SearchResult_04020100(String strStartSerialNo, String strEndSerialNo,
                 ...
                 ref string strMsgID, ref  DataTable dtblResult, string queryType = "S")
         {
             try
             {
                ...
                //* 查詢代償記錄
                //20220222_Ares_Jack_新增LOG紀錄
                BRReport.RecordSQL($"代償證明 查詢已開立的代償證明 [{queryTypeName}]", BRReport.GetFullSqlCmd(sqlcmdSearchResult));
                sw.Start();
                DataSet dstSearchResult = BRSet_Pay_Certify.SearchOnDataSet(sqlcmdSearchResult,
                                intPageIndex, intPageSize, ref intTotalCount);
                sw.Stop();
                ...
                BRReport.RecordSQLExecuteTime($"代償證明 查詢已開立的代償證明 [{queryTypeName}]", sw, dataCount);
                return true;
             }
             catch (Exception exp)
             {
